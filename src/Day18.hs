module Day18 (answer1, answer2) where
...
neighbors g idx = let
  ...
  coordNeighbors = [(x-1, y-1), (x, y-1), (x+1, y-1), (x-1, y), (x+1, y), (x-1, y+1), (x, y+1), (x+1, y+1)]
  indexes = map (coordToIdx n) (filter (\(x,y) -> x>=0 && x<n && y>=0 && y<n) coordNeighbors)
  in map (\idx -> g V.! idx) indexes

nextStatus :: LightStatus -> [LightStatus] -> LightStatus
nextStatus current neighbors = let
  nOnNeighbors = length $ filter (== On) neighbors
  in if current == On
