module Day07 (answer1, answer2) where
...
bitShiftLine = do
  ...
  output <- parseOutput
  return (output, instruction rawI a b)
  where instruction "LSHIFT" = LShift
        instruction _ = RShift

parseLabel = (liftM Label $ some letterChar) <|> (liftM (Val . read) $ some digitChar)
