module Y2016.Day05 (answer1, answer2) where
...
answer1 = do
    ...
    let
        niceWords = filter
            ( \s -> hasThreeVowels s && hasSameLetterTwiceInARow s && not
                (isNaughty s)
            )
            words
...
samePairs _ [] = []
samePairs p (x:xs) =
    if isSamePair p x then (p, x) : samePairs p xs else samePairs p xs
...
hasIntercalated s =
    let pairs = zip3 s (tail s) (drop 2 s)
        hasRepeat (a, _, c) = a == c
