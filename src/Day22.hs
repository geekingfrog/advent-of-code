...
allBattles :: (BattleState -> Spell -> BattleState) -> BattleState -> Rose BattleState
allBattles f s = case battleStatus s of
  Won -> Rose s []
  Lost -> Rose s []
  Ongoing -> let nextStates = map (f s) (castableSpells s)
...
castableSpells s = let
  m = mana s
  missile = if m >= 53 then Just Missile else Nothing
  drain = if m >= 73 then Just Drain else Nothing
  shield = if m >= 113 && shieldTimer s <= 1 then Just Shield else Nothing
  poison = if m >= 173 && poisonTimer s <= 1 then Just Poison else Nothing
  recharge = if m >= 229 && rechargeTimer s <= 1 then Just Recharge else Nothing
  in catMaybes [missile, drain, shield, poison, recharge]
...
carryHardBattle s spell = bossAttack . applyEffect . cast spell . hardModeEffect . applyEffect $ s
...
cast Missile  s  = s {bossHp = bossHp s - 4, mana = mana s - 53, manaSpent = manaSpent s + 53}
cast Drain    s = s {bossHp = bossHp s -2, playerHp = playerHp s + 2, mana = mana s - 73, manaSpent = manaSpent s + 73}
cast Shield   s = s {shieldTimer = 6, mana = mana s - 113, manaSpent = manaSpent s + 113}
cast Poison   s = s {poisonTimer = 6, mana = mana s - 173, manaSpent = manaSpent s + 173}
cast Recharge s = s {rechargeTimer = 5, mana = mana s - 229, manaSpent = manaSpent s + 229}

bossAttack :: BattleState -> BattleState
bossAttack s = let
  armor = if shieldTimer s > 0 then 7 else 0
  dmg = bossAtk s - armor
